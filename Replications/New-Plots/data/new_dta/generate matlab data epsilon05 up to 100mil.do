...
/* Generate the grid of points to evaluate at (equal log spacing of .012) starting from e^5~$148 to e^18.5~$108,254,988*/

gen double z = exp( 5 + ( (_n - 1)*.012) ) if _n<1127												/* Grid for earnings */
...
/* Put in the Pareto tails */
	...
	/* the overall distribution */
		...
		*local alphaz = 2.156																/* This is the alpha found using those between 100K and 150K */
		...
		replace hz = `alphaz'*`zminalpha'*(z^(-(`alphaz'+1))) if z> 150000 & z<110000000		/* The pareto density */
		replace Hz = 1-(`zminalpha'*(z^-`alphaz')) if z>150242 & z<110000000					/* The pareto distribution */
	...
	/* the young distribution */
		...
		*local alphazy = 2.464																/* This is the alpha found using those between 100K and 150K */
		...
		replace hzy = `alphazy'*`zyminalpha'*(z^(-(`alphazy'+1))) if z> 150000 & z<110000000	/* The pareto density */
		replace Hzy = 1-(`zyminalpha'*(z^-`alphazy')) if z>150000 & z<110000000				/* The pareto distribution */
	...
	/* the old distribution */
		...
		*local alphazo = 2.076																/* This is the alpha found using those between 100K and 150K */
		...
		replace hzo = `alphazo'*`zominalpha'*(z^(-(`alphazo'+1))) if z> 150000 & z<110000000	/* The pareto density */
		replace Hzo = 1-(`zominalpha'*(z^-`alphazo')) if z>150000 & z<110000000				/* The pareto distribution */
...
/* Do a local Polynomial regression for the MTR */
	...
	replace mtr = mtr2[736] if _n>736 & _n<1127    											/* There's not enough data far out, so take the tax rate out there as the one it seems to converge to */
...
gen double omegagrid = exp( (_n - 1)*.0125 ) if _n<1127												/* Grid for ability (equal log spacing of .0112) starting from 1 to e^12.152~189,472.7 */
...
local Jomega4k = (1-Jomegao[664])																/* The proportion of people with omega>4000 Note. _n=769 has omegagrid==4001.41 */
...
* NEW: Add an export for z variables, not split by y or o
	...
	save "psidtaxsim_100mil.dta", replace
